...
void GamepadMonitor::sendUDP() {
    ...
    char dontcare = 0;
    updateOutputs();
    char elbowHigh = elbowOutput >> 8;
    char elbowLow = elbowOutput;
    //qDebug() << elbowOutput << " " << (int)leftSideHigh << " " << (int)leftSideLow;
    char shoulderHigh = shoulderOutput >> 8;
    char shoulderLow = shoulderOutput;
    char wristHigh = wristOutput >> 8;
    char wristLow = wristOutput;

    // build the drive control message
    QByteArray out;
    out.append(char(-127));
    out.append(char(1));
    out.append(dontcare);   //pot yaw upper
    out.append(dontcare);   //pot yaw lower
    out.append(shoulderHigh);
    out.append(shoulderLow);
    out.append(elbowHigh);
    out.append(elbowLow);
    out.append(dontcare);   //wrist pitch upper
    out.append(dontcare);   //wrist pitch lower
    out.append(wristHigh);
    out.append(wristLow);
    out.append(dontcare);   //claw open/close
    qint8 hash = (shoulderHigh + shoulderLow + elbowHigh + elbowLow + wristHigh + wristLow)/11;
    out.append(hash);
...
void GamepadMonitor::updateOutputs(){
    if(leftSide > 0){
        elbowOutput += 10;
    }else if(leftSide < 0){
        elbowOutput -= 10;
