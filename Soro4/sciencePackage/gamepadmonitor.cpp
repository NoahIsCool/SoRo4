...
GamepadMonitor::GamepadMonitor( QObject *parent, comms *drillComm)
    : QObject(parent)
    , m_gamepad(0)
{
    this->phi = 0.0;
...
void GamepadMonitor::sendUDP(){
    ...
    if(abs(left_y_axis) > .05){
        if (left_y_axis > 0){
            qDebug() << "extending actuator";
            actuator = 1;// move drill forward
        }
        else {
        ...
        actuator = 0; // stop moving the drill
    }
    ...
    out.append(char(fan));
    char hash = (char)(actuator+spin+fan)/3;
    out.append(hash);
